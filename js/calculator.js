...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (calcSheetBackdrop) {
        calcSheetBackdrop.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeBottomSheet();
        });
        calcSheetBackdrop.addEventListener('touchstart', (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeBottomSheet();
        }, { passive: false });
    ...
    if (calcSheetCloseBtn) {
        calcSheetCloseBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeBottomSheet();
        });
    ...
    // Dismiss bottom sheet when clicking or tapping anywhere outside / Menü dışına tıklandığında veya dokunulduğunda kapatma
    const handleSheetOutsideClick = (e) => {
        if (calcNavList && calcNavList.classList.contains('open')) {
            const isInsideNav = calcNavList.contains(e.target);
            const isInsideTrigger = calcMobileDropdownBtn && calcMobileDropdownBtn.contains(e.target);
            if (!isInsideNav && !isInsideTrigger) {
                closeBottomSheet();
            }
        }
    };
    document.addEventListener('click', handleSheetOutsideClick);
    document.addEventListener('touchend', handleSheetOutsideClick);
